...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "jagoancoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\JagoancoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\JagoancoinCore
    // Mac: ~/Library/Application Support/JagoancoinCore
    // Unix: ~/.jagoancoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "JagoancoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/JagoancoinCore";
#else
    // Unix
    return pathRet / ".jagoancoincore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty jagoancoin.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Jagoancoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Jagoancoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2022, nEndYear) + "The Jagoancoin Core developers";
    }
